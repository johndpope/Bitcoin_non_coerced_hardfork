...
Bitcoin is an adversarial system, and being a technology system, there are always risks in its use.  Being risks, they can only be minimised, they can never be removed.  All risks should be identified, but for some risks, there is no possibility of them being removed, only minimised as much as possible.  Some risks eventuate through the development of new tools that undermine existing systems, some will eventuate through the applicaiton of regulatory pressure, and some will eventuate through the application of political or social pressure.  Some will eventuate through just plain good ol' fashioned stupidity.  There is no way to know.
...
A consensus rule change hard-fork (the new chain) is created and populated with locked coins that may only be unlocked utilizing the private keys of the available for spending in the current old chain UTXO after a specific block depth.  A special transaction is availalbe that allows a node user to generate a key to unlock the coins on the new chain as a function that is the result of a scripted output of an X block-depth (100?) lock transaction script.  The output of this script becomes the key that unlocks value on the the new chain.  

The new chain always has access to the old chain UTXO set but locked to the new chain, with coins available to anyone that is prepared to unlock them in the new chain by using the private keys sourced from in the old chain.  It still can, however, allow transactions to be recorded on the old chain for users that choose not to transact in the new chain.  But the unlocking process on the new chain requires a transaction on the old chain detailing how the coins at that address have been spent on the old chain and are no longer redeemable.  Because the node maintains both the old chain and the new chain UTXO sets, it always 'knows' when funds are available for migration between the old chain and the new chain.  The old chain is extended using old chain mining infrastructre, and the new chain is extended using new chain mining infrastructure.  The migration between the old chain and the new chain acts as a sequential genesis block to the new chain.
